    ...
    >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop-luxury overflow-y-auto">
          <div className="w-full max-w-2xl modal-card-luxury overflow-hidden my-auto max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleAddFee} className="p-6 space-y-6 overflow-y-auto custom-scrollbar-modal">
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2">
                  <label className="mb-2 block text-xs font-extrabold text-foreground">اسم الرسوم <span className="text-danger">*</span></label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="mb-2 block text-xs font-extrabold text-foreground">النوع <span className="text-danger">*</span></label>
                  <input
                    required
                    type="text"
                    list="fee-types-list"
                ...
                <div>
                  <label className="mb-2 block text-xs font-extrabold text-foreground">القيمة ({currency}) <span className="text-danger">*</span></label>
                  <input
                    required
                    type="number"
                    min="0"
                    className="h-11 w-full rounded-xl border border-border/80 bg-background/80 px-4 text-xl font-black text-primary shadow-sm transition-all focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/15 hover:border-primary/50 tabular-nums"
                    value={newFee.amount || ""}
                ...
                <div className="col-span-2 border border-border/60 rounded-2xl p-4 bg-muted/20">
                  <div className="flex items-center gap-2 mb-2">
                    <CheckSquare className="h-4 w-4 text-primary" />
                    <label className="text-sm font-black text-foreground">الاستهداف (صفوف وشعب محددة)</label>
                  </div>
                  <p className="text-xs text-muted-foreground mb-4">
                    إذا تركت الخيارات فارغة، سيتم تطبيق البند على جميع الفصول والشعب في المرحلة تلقائياً.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-4 max-h-52 overflow-y-auto custom-scrollbar-modal p-1">
                    <div className="space-y-2">
                      ...
                      {uniqueGrades.length === 0 ? (
                        ...
                        uniqueGrades.map((g) => (
                          <label key={g} className="flex items-center gap-2.5 cursor-pointer group bg-background/80 p-2.5 rounded-xl border border-border/60 hover:border-primary/50 hover:bg-accent/30 transition-all active:scale-[0.98]">
                            <input 
                              type="checkbox" 
                            ...
                            />
                            <span className="text-xs font-bold group-hover:text-primary transition-colors">{g}</span>
                          </label>
                        ))
                      )}
                    </div>
                    ...
                    <div className="space-y-2 border-r border-border/50 pr-4">
                      ...
                      {targetSections.length === 0 ? (
                        ...
                        [...targetSections].sort((a,b)=>a.grade.localeCompare(b.grade, 'ar') || a.name.localeCompare(b.name,'ar')).map((sec) => (
                          <label key={sec.id} className="flex items-center gap-2.5 cursor-pointer group bg-background/80 p-2.5 rounded-xl border border-border/60 hover:border-primary/50 hover:bg-accent/30 transition-all active:scale-[0.98]">
                            <input 
                              type="checkbox" 
                              className="w-4 h-4 rounded-md border-border text-primary focus:ring-primary"
                            ...
                            <div className="flex flex-col">
                              <span className="text-xs font-bold group-hover:text-primary transition-colors">شعبة {sec.name}</span>
                              <span className="text-[10px] text-muted-foreground">{sec.grade}</span>
                            </div>
                          </label>
                        ))
                      )}
                ...
                <div className="col-span-2 flex items-center justify-between p-4 border border-border/60 rounded-2xl bg-muted/20">
                  <div>
                    <label className="font-extrabold block text-sm text-foreground">رسوم إلزامية</label>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-border/50 shrink-0">
                <button 
                  type="button" 
                  onClick={() => setIsModalOpen(false)}
                  className="rounded-xl px-5 py-2.5 text-sm font-bold hover:bg-accent transition-colors border border-border/80 active:scale-[0.98]"
                >
                  إلغاء
                </button>
                <button 
